...
local PersonInfor	= class("PersonInfor", cc.load("mvc").ViewBase)

function PersonInfor:onConfig()
	self.widget			= {
...
function PersonInfor:onEnter()
	
end

function PersonInfor:onExit()
	cc.UserDefault:getInstance():setBoolForKey('PDKOpenUserEffect', self.isOpen)
end

function PersonInfor:onCreate(param)
	local data = param[1]
...
function PersonInfor:updateInfo( data )
...
function PersonInfor:updateControl( ... )
...
function PersonInfor:onControl()
    --是否开启
    self.isOpen = not self.isOpen
    self:updateControl()
end

function PersonInfor:getPlayerInfoByUserID(dwUserID)
...
function PersonInfor:updateScrollviewImage()	
...
function PersonInfor:buttonCall(sender)
...
function PersonInfor:addListener(btn, callback)

...
function ChatLayer:onCreate(params)
...
function ChatLayer:initEmSetting(...)
	--50 为了做分页给每页表情赋值一个id
	self:initOneEmjio('page1_Button_1', 'page1_ScrollView_1', 'press_1', 23, 0)
end

--初始化表情 --page1_ScrollView_1
function ChatLayer:initOneEmjio(btnName, listName, press, index, start)
...
--初始化 文本 从100开始
function ChatLayer:initChatLab(...)
...
function ChatLayer:addMaskListen(...)
...
function ChatLayer:addPanelListen(item, call)
...
function ChatLayer:closeView(...)
	self:setVisible(false)
end

function ChatLayer:onSendCall(...)
...
function ChatLayer:clickExpressLab(sender)
	local chat = require("game.puke.ChatConfig")
...
function ChatLayer:onClickHead(sender)
	local name = sender:getName()
	
	self:showPage(name)
end

function ChatLayer:showPage( hedeName )
...
function ChatLayer:emButtonCall(sender)
	local name = sender:getName()
	self:showPage(name)
end
...
function ChatLayer:SUB_GR_SEND_CHAT(event)

end

function ChatLayer:SUB_GF_USER_EXPRESSION(event)
